...
export function WriterApp() {
  ...
  return <div className="appShell">
    ...
    {menu!=="none" && <div className="modalBackdrop" onMouseDown={()=>setMenu("none")}><section className="modal" onMouseDown={e=>e.stopPropagation()}><button className="modalClose" onClick={()=>setMenu("none")}>×</button>
      ...
      {menu==="account" && <><p className="eyebrow">CLOUD SYNC</p><h2>クラウド同期</h2><div className="syncState"><span>●</span><div><b>{user ? `${user.email} でログイン中` : "端末内保存は有効です"}</b><p>通信に失敗しても、原稿はIndexedDBに残ります。</p></div></div>{user ? <><button className="primary wide" onClick={syncAll}>今すぐすべて同期</button><button className="secondary wide" onClick={()=>supabase?.auth.signOut()}>ログアウト</button></> : <><label>メールアドレス<input value={email} onChange={e=>setEmail(e.target.value)} type="email" placeholder="you@example.com"/></label><label>パスワード<input value={password} onChange={e=>setPassword(e.target.value)} type="password" minLength={8}/></label><button className="primary wide" onClick={()=>signIn(false)}>ログイン</button><button className="secondary wide" onClick={()=>signIn(true)}>新規登録</button></>}<p className="finePrint">{cloudMessage || (supabase ? "同期は入力停止から約2秒後に行われます。" : "管理者がSupabase接続情報を設定すると利用できます。")}</p></>}
    </section></div>}
  </div>;
}
